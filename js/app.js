...
function renderReportTable() {
    ...
    const tableHeader = `<thead class="bg-gray-100 text-gray-600 text-xs uppercase font-bold text-left sticky top-0 z-10"><tr><th class="px-4 py-1.5 border-b-2 border-gray-200 bg-gray-100">Antrian</th><th class="px-4 py-1.5 border-b-2 border-gray-200 bg-gray-100">Waktu</th><th class="px-4 py-1.5 border-b-2 border-gray-200 bg-gray-100">Pembeli</th><th class="px-4 py-1.5 w-1/3 border-b-2 border-gray-200 bg-gray-100">Detail Item</th><th class="px-4 py-1.5 border-b-2 border-gray-200 bg-gray-100">Metode</th><th class="px-4 py-1.5 text-right border-b-2 border-gray-200 bg-gray-100">Total</th></tr></thead>`;
    const tableRows = currentData.map((tx, index) => {
        ...
        const receiptBtn = isPrintingMode ? '' : `<button onclick="printReceipt('${tx.id}')" class="mt-1 bg-gray-800 text-white text-[9px] font-bold px-1 py-0.5 rounded hover:bg-black shadow active:scale-95 flex items-center gap-1 w-full justify-center">🧾 CETAK RESI</button>`;
        return `<tr class="${rowColor} border-b border-gray-200 hover:bg-gray-100 transition group"><td class="px-4 py-1.5 text-bebyte-purple align-top text-center"><div class="h-7 flex items-center justify-center">${queueDisplay}</div>${actionBtn}</td><td class="px-4 py-1.5 text-xs font-medium text-gray-500 align-top whitespace-nowrap">${new Date(tx.id).toLocaleTimeString('id-ID')}<br><span class="text-[10px]">${new Date(tx.id).toLocaleDateString('id-ID')}</span></td><td class="px-4 py-1.5 align-top"><div class="font-bold text-sm text-black uppercase truncate max-w-[120px]">${tx.customer.name}</div>${noteDisplay}</td><td class="px-4 py-1.5 align-top"><div class="max-h-[100px] overflow-y-auto custom-scroll pr-1">${itemsSummary}</div></td><td class="px-4 py-1.5 text-xs align-top text-center"><div class="h-7 flex items-center justify-center">${methodBadge}</div>${payBtn}${finishBtn}</td><td class="px-4 py-1.5 text-sm font-bold text-black text-right align-top"><div class="h-7 flex items-center justify-end">${fmt(tx.total)}</div>${receiptBtn}</td></tr>`;
    }).join('');
